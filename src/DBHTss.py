...
######################################################################
def complete(dd):
    return np.max(dd[dd != 0], axis=0)

def single(dd):
    return np.min(dd[dd != 0], axis=0)
...
def DBHTs(D, S, method, leaf_order=True):
    r"""
    ...
    Z = HierarchyConstruct4s(Rpm, Dpm, T8, Adjv, Mv, method)
...
def HierarchyConstruct4s(Rpm, Dpm, Tc, Adjv, Mv, method):
    r"""
    ...
    for n in range(0, len(kvec)):
        ...
        for b in range(0, len(Bub) - 1):
            (PairLink, dvu) = LinkageFunction(dpm, LabelVec, method)
    ...
    for n in range(0, len(kvec) - 1):
        (PairLink, dvu) = LinkageFunction(Dpm, LabelVec1, method)
        # %[PairLink,dvu]=LinkageFunction(Rpm,LabelVec);
...
def LinkageFunction(d, labelvec, method):
    if method == 'ward':
        return WardFunction(d, labelvec)
    ...
    for r in range(0, len(lvec) - 1):
        ...
        for c in range(r + 1, len(lvec)):
            ...
            if dd[dd != 0].shape[0] == 0:
                ...
                # linkage distance
                Link1 = np.hstack((lvec[r], lvec[c], linkage[method](dd)))
...
def WardFunction(d, labelvec):
    ...
    for r in range(0, len(lvec) - 1):
        ...
        for c in range(r + 1, len(lvec)):
            ...
            dr = d[np.ix_(vecr, vecr)]
            dc = d[np.ix_(vecc, vecc)]
            dd = d[np.ix_(x1, x1)]
            ...
            if dd[dd != 0].shape[0] == 0:
                ...
                # linkage distance
                lr, lc, ld = 2*vecr.sum(), 2*vecc.sum(), 2*(vecr.sum()+vecc.sum())
                Link1 = np.hstack((lvec[r], lvec[c], np.sum(dd**2)/ld-np.sum(dc**2)/lc-np.sum(dr**2)/lr))
            Links = np.vstack((Links, Link1))
            del vecc
